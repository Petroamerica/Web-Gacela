...
export default class Tabla extends Component {
    ...
    exportExcel = async () =>{
        ...
        let precioTotal = 0.000
        worksheet.columns = [  
            { width: 12 }, { width: 15 }, { width: 15 }, { width: 15 }, { width: 12 }, { width: 12 },{ width: 15 }, { width: 40 }, { width: 15 }, { width: 27 }, { width: 15 }, { width: 50 }, { width: 30 }, { width: 20 }, { width: 35 }
          ];
        for (let index = 0; index < servicios.length; index++) {   
            let precioServicio = 0.000
            ...
            worksheet.getCell('C'+cliente).value = servicios[index].cabecera.hora_llegada
            worksheet.getCell('C'+cliente).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('C'+cliente).alignment = { vertical: 'middle', horizontal: 'center' };

            worksheet.getCell('H'+cliente).value = servicios[index].cabecera.id_chofer
            worksheet.getCell('H'+cliente).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('H'+cliente).alignment = { vertical: 'middle', horizontal: 'center' };

            worksheet.getCell('J'+cliente).value = servicios[index].cabecera.id_unidad
            worksheet.getCell('J'+cliente).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('J'+cliente).alignment = { vertical: 'middle', horizontal: 'center' };
            ...
            */
            ...
            worksheet.getCell('J'+titulo).value = 'ESTADO DE MOVILIZACIÓN';
            ...
            worksheet.getCell('M'+titulo).value = 'DISTRITO';
            worksheet.getCell('M'+titulo).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('M'+titulo).alignment = { vertical: 'middle', horizontal: 'center' };
            worksheet.getCell('M'+titulo).fill = { type: 'pattern', pattern:'darkTrellis', fgColor:{argb:'FFF323'}, bgColor:{argb:'FFEFA0'} };

            worksheet.getCell('N'+titulo).value = 'COSTO DE SERVICIO';
            worksheet.getCell('N'+titulo).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('N'+titulo).alignment = { vertical: 'middle', horizontal: 'center' };
            worksheet.getCell('N'+titulo).fill = { type: 'pattern', pattern:'darkTrellis', fgColor:{argb:'FFF323'}, bgColor:{argb:'FFEFA0'} };

            worksheet.getCell('O'+titulo).value = 'COMENTARIO';
            worksheet.getCell('O'+titulo).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            worksheet.getCell('O'+titulo).alignment = { vertical: 'middle', horizontal: 'center' };
            worksheet.getCell('O'+titulo).fill = { type: 'pattern', pattern:'darkTrellis', fgColor:{argb:'FFF323'}, bgColor:{argb:'FFEFA0'} };
            ...
            for (let index2 = 0; index2 < servicios[index].data.length; index2++) {
                ...
                */
                ...
                worksheet.getCell('J'+detalle).value = servicios[index].data[index2].id_tipo_estado_mov;
                ...
                worksheet.getCell('K'+detalle).value = servicios[index].data[index2].celular;
                worksheet.getCell('K'+detalle).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
                worksheet.getCell('K'+detalle).alignment = { vertical: 'middle', horizontal: 'center' };

                worksheet.getCell('L'+detalle).value = servicios[index].data[index2].direccion;
                worksheet.getCell('L'+detalle).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };

                worksheet.getCell('M'+detalle).value = servicios[index].data[index2].descripcion_distrito;
                worksheet.getCell('M'+detalle).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
                worksheet.getCell('M'+detalle).alignment = { vertical: 'middle', horizontal: 'center' };

                worksheet.getCell('N'+detalle).value = servicios[index].data[index2].costo_pasajero;
                worksheet.getCell('N'+detalle).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
                worksheet.getCell('N'+detalle).alignment = { vertical: 'top', horizontal: 'right' };

                worksheet.getCell('O'+detalle).value = servicios[index].data[index2].comentario;
                worksheet.getCell('O'+detalle).border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
                worksheet.getCell('O'+detalle).alignment = { vertical: 'top', horizontal: 'right' };

                detalle=detalle+1
                precioServicio = parseFloat(servicios[index].data[index2].costo_pasajero) + parseFloat(precioServicio)
                precioTotal = parseFloat(servicios[index].data[index2].costo_pasajero) + parseFloat(precioTotal)
                if((index2+1) === servicios[index].data.length){
                    worksheet.getCell('L'+detalle).value = "COSTO DE SERVICIO";
                    worksheet.getCell('L'+detalle).font = { bold: true };
                    worksheet.getCell('M'+detalle).value = precioServicio.toFixed(3);
                    worksheet.getCell('M'+detalle).alignment = { vertical: 'top', horizontal: 'right' };
                    worksheet.getCell('M'+detalle).font = { bold: true };
                }
            }
            titulo = detalle+2
            detalle = titulo+1
            cliente = titulo-1
            if((index+1) === servicios.length){
                worksheet.getCell('L'+detalle).value = "COSTO TOTAL";
                worksheet.getCell('L'+detalle).font = { bold: true };
                worksheet.getCell('M'+detalle).value = precioTotal.toFixed(3);
                worksheet.getCell('M'+detalle).alignment = { vertical: 'top', horizontal: 'right' };
                worksheet.getCell('M'+detalle).font = { bold: true };
            }
        }
        ...
        return (
            <div style={{overflowY: 'scroll',  maxHeight: '900px', marginTop: '15px', padding:'0px 15px', marginBottom:'15px'}}>
                <div style={{display:'flex', justifyContent:'space-between', fontSize: '1.2em', fontWeight:'bold', marginBottom: '10px'}}>
                    ...
                    <div>
                        <span style={{color:'green'}}>Estado de la liquidacion: ACTIVO -</span> &nbsp;&nbsp;&nbsp;&nbsp; <span style={{color:'red'}}>Costo: { parseFloat(this.props.costoTotal).toFixed(3)}</span>
                    </div>
                </div>
                        ...
                        <tbody key={key}>
                            <tr>
                                <td colSpan="12" style={{padding:'5px', fontSize:'0.8em'}}>
                                    ...
                                    <span style={{marginLeft: '15px'}}>Automovil: <b>{value.cabecera.id_unidad}</b></span>
                                    <span style={{marginLeft: '15px'}}>Chofer: <b>{value.cabecera.id_chofer}</b></span>
                                </td>
                            </tr>
                            {value.data.map((subValue, subKey) => 
                                ...
                                <>      
                                    ...
                                    <td style={{maxWidth:"250px", height:'30px', padding:' 5px 4px'}} align="center">
                                    ...
                                    <td style={{width:"30px", padding:"4px", fontSize: '0.7em'}} align="center" >
                                        {
                                            subValue.costo_pasajero !== null ? 
                                            parseFloat(subValue.costo_pasajero).toFixed(3) : '-'
                                        }
                                    </td>
                                    <td style={{width:"30px", padding:"4px", fontSize: '0.7em'}} align="center" >{subValue.id_tipo_estado_mov}</td>
                                </>
                                </tr>   
                            )}
                        </tbody>
